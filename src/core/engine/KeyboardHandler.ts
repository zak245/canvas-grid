...
export class KeyboardHandler {
    ...
    handleKeyDown = (e: KeyboardEvent) => {
        ...
        if ((e.metaKey || e.ctrlKey) && e.key === 'z') {
            e.preventDefault();
            if (e.shiftKey) {
                if (this.engine.history.canRedo()) {
                    this.engine.history.redo();
                }
            } else {
                if (this.engine.history.canUndo()) {
        ...
        // If editing, trap focus/navigation
        // This safeguards against focus leaking back to the canvas while the editor is open
        if (editingCell) {
            if (e.key === 'Enter') {
                // Commit
                ...
                // Cancel
                this.engine.stopEdit(true);
                e.preventDefault();
                e.stopPropagation();
            } else if (['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight', 'Tab', 'Home', 'End', 'PageUp', 'PageDown'].includes(e.key)) {
                // Trap navigation keys to prevent grid movement while editing
                // This forces the user to click the input if it missed focus
                e.preventDefault();
                e.stopPropagation();
            }
            return;
        ...
        // Primary is guaranteed non-null after the check above
        const primary = selection.primary;
        const { row, col } = primary;
        // Use View Layer for row count
        ...
        // Enter: Start Editing (Only if not already editing)
        else if (e.key === 'Enter') {
    ...
    // Public helpers for Clipboard Manager / Context Menu
    public getSelectedDataAsTSV(): string | null {
        const selection = this.engine.store.getState().selection;
        if (!selection || selection.ranges.length === 0) return null;

        const rows = this.engine.rows.getViewRows(); // Use View Layer
        const columns = this.engine.model.getColumns();
        const rowCount = this.engine.rows.getRowCount();
        ...
        for (let r = minRow; r <= maxRow; r++) {
            ...
            const row = rows[r]; // Use visual row index
    ...
    public async performCopy(): Promise<void> {
        const tsv = this.getSelectedDataAsTSV();
        if (tsv) {
            try {
    ...
    public async performCut(): Promise<void> {
        await this.performCopy();
        this.performDelete();
    }

    public async performPaste(): Promise<void> {
        try {
    ...
    onCopy(e: ClipboardEvent) {
        ...
        if (tsv) {
             ...
             console.log(`Copied approx ${rows} rows`);
        }
    }
    ...
    private clearSelectedCells() {
        ...
        // Create batch update for clear
        const updates = [];

        for (let r = range.start.row; r <= range.end.row; r++) {
            for (let c = range.start.col; c <= range.end.col; c++) {
                const col = columns[c];
                // Note: using bulkUpdateCells which uses RowManager logic via engine
                updates.push({ rowIndex: r, columnId: col.id, value: '' });
            }
        }
        
        this.engine.bulkUpdateCells(updates).catch(console.error);
    }
}
